...
# ---------- Dataset: 每张图 → 两条样本 (type=50,value=b50) & (type=800,value=b800) ----------
class TypeValSidecarDataset(Dataset):
    def __init__(self, root, image_size=512):
        ...
        # 预先建立 index 映射：每个 txt 生成两个条目
        self.items = []  # (img_path, type_id, value)
        for t in self.txts:
            ...
            # 两条训练条目
            self.items.append((img_path, 0, b50))   # 0→50
            self.items.append((img_path, 1, b800))  # 1→800

    def __len__(self): return len(self.items)

    def __getitem__(self, i):
        img_path, type_id, value = self.items[i]
        img = Image.open(img_path).convert("RGB")
        img = self.tf(img)
        return {
            "pixel_values": img,             # [3,H,W] 归一化到 [-1,1]
            "b_type_id": torch.tensor(type_id, dtype=torch.long),  # 0 or 1
...
def main():
    ...
    out_dir=os.environ.get("OUT_DIR","/workspace/experiments/typeval_enc_lora")
    ...
    # 3) dataset / per-type stats
    ds = TypeValSidecarDataset(train_dir, image_size=512)
    assert len(ds) > 0, "Empty dataset"
    # 逐类型统计 mean/std
    vals_by_type = {0:[], 1:[]}
    for it in ds.items:
        _, tid, val = it
        vals_by_type[tid].append(val)
    mean0 = float(np.mean(vals_by_type[0])); std0 = float(np.std(vals_by_type[0]) + 1e-6)
    mean1 = float(np.mean(vals_by_type[1])); std1 = float(np.std(vals_by_type[1]) + 1e-6)

    enc = TypeValEncoder(seq_len=16, hidden_size=pipe.text_encoder.config.hidden_size, num_types=2).to(device)
    enc.set_norm_per_type([mean0, mean1], [std0, std1])
    enc.train()
    ...
    while step < max_steps:
        for batch in dl:
            type_ids = batch["b_type_id"].to(device)     # [B]
            values   = batch["value"].to(device)         # [B]
            ...
            # 条件序列 (B,L,768)
            text_emb = enc(type_ids, values)                 # fp32
            text_emb = text_emb.to(noisy_latents.dtype)      # 对齐 half
            ...
            if step % SAVE_EVERY == 0:
                ...
                torch.save(enc.state_dict_light(), os.path.join(out_dir, f"typeval_enc_step{step}.pt"))

            if step >= max_steps:
                break

    pipe.unet.save_attn_procs(out_dir)
    torch.save(enc.state_dict_light(), os.path.join(out_dir, "typeval_enc_final.pt"))
    print(f"[OK] saved LoRA + TypeValEncoder to {out_dir}")
