import os, glob, re, argparse, warnings, numpy as np
from PIL import Image
warnings.filterwarnings("ignore")

_NUM_RE = r"([-+]?\d[\d,]*(?:\.\d+)?(?:[eE][-+]?\d+)?)"

def parse_value(txt, key):
    m = re.search(rf"{key}\s*=\s*{_NUM_RE}\s*[,;]?", txt)
    if not m: return None
    s = m.group(1).rstrip(",;").replace(",", "")
    try: return float(s)
    except: return None

def read_pairs(val_dir, fake_dir, key_field):
    txts = sorted(glob.glob(os.path.join(val_dir, "*.txt")))
    pairs=[]
    for t in txts:
        stem = os.path.splitext(os.path.basename(t))[0]
        with open(t,"r",encoding="utf-8") as f: s=f.read().strip()
        v = parse_value(s, f"signal_{key_field}")
        ...
        pairs.append((real_img,fake_img,v))
    return pairs
...
def feats_clip(imgs):
    try:
        ...
        for im in imgs:
            ...
            rgb_mean=arr.mean((0,1)); rgb_std=arr.std((0,1))
            h=[]
            for c in range(3):
                h.append(np.histogram(arr[...,c], bins=16, range=(0,1), density=True)[0])
            X.append(np.concatenate([rgb_mean, rgb_std, *h],0))
        return np.stack(X,0), "fallback_hist"

def onetarget_consistency(X_real, X_fake, y):
    ...
    return r_real, rmse_real, r_fake_p, r_fake_s, rmse_fake

def main():
    ap=argparse.ArgumentParser()
    ap.add_argument("--val_dir", required=True)
    ap.add_argument("--fake_dir", required=True)
    ap.add_argument("--key", choices=["b50","b800"], required=True)
    args=ap.parse_args()

    key_field = "signal_b50" if args.key=="b50" else "signal_b800"
    pairs = read_pairs(args.val_dir, args.fake_dir, key_field)
    if not pairs: raise RuntimeError("No matched pairs.")
    ...
    bstats = onetarget_consistency(X_real, X_fake, y)

    print("\n=== VALUE CONSISTENCY (single) ===")
    print("key:", args.key)
    print("pairs_used:", len(pairs))
    print("feat_backend:", backend)
    print("real_test_pearson_r:", float(0 if np.isnan(bstats[0]) else bstats[0]))
    print("real_test_rmse:", float(bstats[1]))
    print("fake_pearson_r:", float(0 if np.isnan(bstats[2]) else bstats[2]))
    print("fake_spearman_r:", float(0 if np.isnan(bstats[3]) else bstats[3]))
    print("fake_rmse:", float(bstats[4]))
