...
class SidecarDataset(Dataset):
    """
    Read grayscale image + sidecar txt containing THREE numeric values.

    Sidecar format examples:
    ...
    def __init__(self, root, image_size=256):
        ...
        if len(self.txts) == 0:
            raise FileNotFoundError(f"No .txt sidecars found in {root}")

    def _parse_triplet(self, text: str):
        """
        Return (v1, v2, v3) as floats, or (None,None,None) if parse fails.
        """
        if not text:
    ...
    def __getitem__(self, i):
        ...
        try:
            ...
            b50, b800, b2000 = self._parse_triplet(s)
            if b50 is None:
                print(f"[WARN] Parse fail: {tpath} | content='{s[:80]}'")
                b50, b800, b2000 = 0.0, 0.0, 0.0
            ...
            # Fail-safe sample (use configured image_size)
            return {
                "pixel_values": torch.zeros(1, self.image_size, self.image_size, dtype=torch.float32),
                "cond": torch.tensor([0.0, 0.0, 0.0], dtype=torch.float32),
            }
...
def main():
    ...
    while step < max_steps:
        ...
        for batch in dl:
            ...
            cond = batch["cond"].to(device, dtype=torch.float32)          # [B,3]
            bsz = imgs.size(0)
